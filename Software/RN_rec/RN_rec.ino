...
//25/05/2017 Dylan Brenneis, McNiel Keri
...
//LEDs
int yel_led_1 = 40;  //Initialize LEDs for feedback of filter stabilization
int yel_led_2 = 42;
int yel_led_3 = 44;
int yel_led_4 = 46;
int grn_led = 48;
  ...
  if (Serial1.available() >= sizeof(recieveBuffer)) // If there's anything to read
  {
    if ( Serial1.read() * 256 + Serial1.read() == FLAG) {
      ...
      if (Serial1.read() * 256 + Serial1.read() == FLAG) {
        ...
        D0_pos = map(D0_pos , 450, 700, 180, 0); //scale it to use it with the servo (value between 0 and 180)
        D1_pos = map(D1_pos , 350, 700, 180, 0);
        D2_pos = map(D2_pos , 450, 900, 180, 0);
        D3_pos = map(D3_pos , 350, 750, 180, 0);
        D4_pos = map(D4_pos , 400, 800, 180, 0);
        D5_pos = map(D5_pos , 400, 800, 180, 0);
